...
Summary statistics support this: the mean TotalXP under Double XP + 10% was significantly higher, with greater spread, compared to the 10% Boost group.
...
```
To model Score, we first used LASSO regression with 10-fold cross-validation. The LASSO model selected all six predictors: total_xp, eliminations, deaths, damage, xp_multiplier, and winning_team. None of the variables were eliminated, although the damage coefficient was relatively small compared to the others. We also performed backward stepwise regression using AIC as the selection criterion. Backward stepwise selected five predictors: total_xp, eliminations, deaths, xp_multiplier, and winning_team, removing damage from the model. Comparing the two methods, we observe that LASSO tends to shrink smaller coefficients toward zero but kept damage in the model, while backward stepwise removed it completely based on AIC considerations. Overall, both methods consistently identified total_xp, eliminations, deaths, xp_multiplier, and winning_team as important predictors for modeling Score.
...
cod_q2_standardized <- cod_q2 %>%
  mutate(
    total_xp = scale(total_xp),
    eliminations = scale(eliminations),
    deaths = scale(deaths),
    xp_multiplier = scale(xp_multiplier),
    winning_team = scale(winning_team)
  )
